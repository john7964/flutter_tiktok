...
import 'package:flutter/material.dart' show FloatingActionButtonLocation, FloatingActionButtonAnimator, MaterialBanner, MaterialBannerClosedReason, SnackBar, SnackBarClosedReason, FloatingActionButton, DrawerCallback, DrawerControllerState, ThemeData, SnackBarBehavior, MaterialBannerThemeData, standardEasing, debugCheckHasScaffoldMessenger, kFloatingActionButtonSegue, kFloatingActionButtonTurnInterval, BottomSheet, Colors, DrawerAlignment, DrawerController, Theme, AppBar, FlexibleSpaceBar, MaterialBannerTheme, Divider, Material, ScaffoldPrelayoutGeometry, MaterialType;
...
/// Manages [SnackBar]s and [MaterialBanner]s for descendant [TransparencyScaffold]s.
///
...
///
/// When the [ScaffoldMessenger] has nested [TransparencyScaffold] descendants, the
...
class ScaffoldMessenger extends StatefulWidget {
  /// Creates a widget that manages [SnackBar]s for [TransparencyScaffold] descendants.
...
/// [showMaterialBanner] for every registered [TransparencyScaffold] that is a descendant of
...
class ScaffoldMessengerState extends State<ScaffoldMessenger> with TickerProviderStateMixin {
  final LinkedHashSet<TransparencyScaffoldState> _scaffolds = LinkedHashSet<TransparencyScaffoldState>();
  ...
  void _register(TransparencyScaffoldState scaffold) {
  ...
  void _unregister(TransparencyScaffoldState scaffold) {
  ...
  void _updateScaffolds() {
    for (final TransparencyScaffoldState scaffold in _scaffolds) {
      if (_isRoot(scaffold)) {
        scaffold._updateSnackBar();
        scaffold._updateMaterialBanner();
      }
  ...
  bool _isRoot(TransparencyScaffoldState scaffold) {
    final TransparencyScaffoldState? parent = scaffold.context.findAncestorStateOfType<TransparencyScaffoldState>();
    return parent == null || !_scaffolds.contains(parent);
  }

  // SNACKBAR API

  /// Shows a [SnackBar] across all registered [TransparencyScaffold]s. Scaffolds register
  ...
  /// positioned above the widgets provided to [TransparencyScaffold.floatingActionButton],
  /// [TransparencyScaffold.persistentFooterButtons], and [TransparencyScaffold.bottomNavigationBar].
  ...
  ///
  /// {@tool dartpad}
  /// Here is an example showing that a floating [SnackBar] appears above [TransparencyScaffold.floatingActionButton].
  ///
  ...
  /// Shows a [MaterialBanner] across all registered [TransparencyScaffold]s. Scaffolds register
...
///
/// [TransparencyScaffoldState] uses this to seamlessly change transition animations
...
/// Geometry information for [TransparencyScaffold] components after layout is finished.
///
/// To get a [ValueNotifier] for the scaffold geometry of a given
/// [BuildContext], use [TransparencyScaffold.geometryOf].
///
...
/// [FloatingActionButton].
///
/// For information about the [TransparencyScaffold]'s geometry that is used while laying
/// out the [FloatingActionButton], see [ScaffoldPrelayoutGeometry].
@immutable
class ScaffoldGeometry {
  /// Create an object that describes the geometry of a [TransparencyScaffold].
  const ScaffoldGeometry({this.bottomNavigationBarTop, this.floatingActionButtonArea});

  /// The distance from the [TransparencyScaffold]'s top edge to the top edge of the
  /// rectangle in which the [TransparencyScaffold.bottomNavigationBar] bar is laid out.
  ///
  /// Null if [TransparencyScaffold.bottomNavigationBar] is null.
  final double? bottomNavigationBarTop;

  /// The [TransparencyScaffold.floatingActionButton]'s bounding rectangle.
  ///
...
/// * Motion animations, which are triggered by the [TransparencyScaffold]
...
/// [TransparencyScaffoldState] for the current [BuildContext] via [TransparencyScaffold.of] and use the
/// [TransparencyScaffoldState.showBottomSheet] function.
///
/// {@tool dartpad}
/// This example shows a [TransparencyScaffold] with a [body] and [FloatingActionButton].
/// The [body] is a [Text] placed in a [Center] in order to center the text
/// within the [TransparencyScaffold]. The [FloatingActionButton] is connected to a
...
///
/// {@tool dartpad}
/// This example shows a [TransparencyScaffold] with a blueGrey [backgroundColor], [body]
/// and [FloatingActionButton]. The [body] is a [Text] placed in a [Center] in
/// order to center the text within the [TransparencyScaffold]. The [FloatingActionButton]
...
///
/// {@tool dartpad}
/// This example shows a [TransparencyScaffold] with an [AppBar], a [BottomAppBar] and a
/// [FloatingActionButton]. The [body] is a [Text] placed in a [Center] in order
/// to center the text within the [TransparencyScaffold]. The [FloatingActionButton] is
...
///
/// If [TransparencyScaffold.bottomSheet] is a [DraggableScrollableSheet],
/// [TransparencyScaffold.floatingActionButton] is set, and the bottom sheet is dragged to
/// cover greater than 70% of the Scaffold's height, two things happen in parallel:
///
///   * Scaffold starts to show scrim (see [TransparencyScaffoldState.showBodyScrim]), and
///   * [TransparencyScaffold.floatingActionButton] is scaled down through an animation with a [Curves.easeIn], and
///     disappears when the bottom sheet covers the entire Scaffold.
///
/// And as soon as the bottom sheet is dragged down to cover less than 70% of the [TransparencyScaffold], the scrim
/// disappears and [TransparencyScaffold.floatingActionButton] animates back to its normal size.
///
/// ## Troubleshooting
///
/// ### Nested Scaffolds
...
///    using the [TransparencyScaffoldState.showBottomSheet] method, or modal, in which case
...
///  * [TransparencyScaffoldState], which is the state associated with this widget.
///  * <https://material.io/design/layout/responsive-layout-grid.html>
///  * Cookbook: [Add a Drawer to a screen](https://docs.flutter.dev/cookbook/design/drawer)
class TransparencyScaffold extends StatefulWidget {
  /// Creates a visual scaffold for Material Design widgets.
  const TransparencyScaffold({
    super.key,
    this.appBar,
    this.body,
  ...
  ///
  /// If null, the [TransparencyScaffoldState] will use the default location, [FloatingActionButtonLocation.endFloat].
  final FloatingActionButtonLocation? floatingActionButtonLocation;

  /// Animator to move the [floatingActionButton] to a new [floatingActionButtonLocation].
  ///
  /// If null, the [TransparencyScaffoldState] will use the default animator, [FloatingActionButtonAnimator.scaling].
  ...
  /// Typically a [Drawer].
  ///
  /// To open the drawer, use the [TransparencyScaffoldState.openDrawer] function.
  ///
  /// To close the drawer, use either [TransparencyScaffoldState.closeDrawer], [Navigator.pop]
  /// or press the escape key on the keyboard.
  ///
  /// {@tool dartpad}
  /// To disable the drawer edge swipe on mobile, set the
  /// [TransparencyScaffold.drawerEnableOpenDragGesture] to false. Then, use
  /// [TransparencyScaffoldState.openDrawer] to open the drawer and [Navigator.pop] to close
  /// it.
  ///
  ...
  /// Optional callback that is called when the [TransparencyScaffold.drawer] is opened or closed.
  ...
  /// Typically a [Drawer].
  ///
  /// To open the drawer, use the [TransparencyScaffoldState.openEndDrawer] function.
  ///
  /// To close the drawer, use either [TransparencyScaffoldState.closeEndDrawer], [Navigator.pop]
  /// or press the escape key on the keyboard.
  ///
  /// {@tool dartpad}
  /// To disable the drawer edge swipe, set the
  /// [TransparencyScaffold.endDrawerEnableOpenDragGesture] to false. Then, use
  /// [TransparencyScaffoldState.openEndDrawer] to open the drawer and [Navigator.pop] to
  /// close it.
  ///
  ...
  /// Optional callback that is called when the [TransparencyScaffold.endDrawer] is opened or closed.
  ...
  /// Determines if the [TransparencyScaffold.drawer] can be opened with a drag
  /// gesture on mobile.
  ///
  ...
  /// Determines if the [TransparencyScaffold.endDrawer] can be opened with a
  /// gesture on mobile.
  ///
  /// On desktop platforms, the drawer is not draggable.
  ///
  ...
  /// Restoration ID to save and restore the state of the [TransparencyScaffold].
  ///
  ...
  /// Finds the [TransparencyScaffoldState] from the closest instance of this class that
  ...
  ///
  /// {@tool dartpad}
  /// Typical usage of the [TransparencyScaffold.of] function is to call it from within the
  /// `build` method of a child of a [TransparencyScaffold].
  ///
  /// ** See code in examples/api/lib/material/scaffold/scaffold.of.0.dart **
  /// {@end-tool}
  ///
  /// {@tool dartpad}
  /// When the [TransparencyScaffold] is actually created in the same `build` function, the
  /// `context` argument to the `build` function can't be used to find the
  /// [TransparencyScaffold] (since it's "above" the widget being returned in the widget
  ...
  /// [TransparencyScaffold]. In this solution, you would have an outer widget that creates
  /// the [TransparencyScaffold] populated by instances of your new inner widgets, and then
  /// in these inner widgets you would use [TransparencyScaffold.of].
  ///
  /// A less elegant but more expedient solution is assign a [GlobalKey] to the
  /// [TransparencyScaffold], then use the `key.currentState` property to obtain the
  /// [TransparencyScaffoldState] rather than using the [TransparencyScaffold.of] function.
  ///
  /// If there is no [TransparencyScaffold] in scope, then this will throw an exception.
  /// To return null if there is no [TransparencyScaffold], use [maybeOf] instead.
  static TransparencyScaffoldState of(BuildContext context) {
    final TransparencyScaffoldState? result = context.findAncestorStateOfType<TransparencyScaffoldState>();
    if (result != null) {
      return result;
    }
    throw FlutterError.fromParts(<DiagnosticsNode>[
  ...
  ///    documentation.
  static TransparencyScaffoldState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<TransparencyScaffoldState>();
  }

  /// Returns a [ValueListenable] for the [ScaffoldGeometry] for the closest
  /// [TransparencyScaffold] ancestor of the given context.
  ///
  ...
  /// paint pass.
  ///
  /// The closest [TransparencyScaffold] ancestor for the context might change, e.g when
  /// an element is moved from one scaffold to another. For [StatefulWidget]s
  /// using this listenable, a change of the [TransparencyScaffold] ancestor will
  /// trigger a [State.didChangeDependencies].
  ///
  /// A typical pattern for listening to the scaffold geometry would be to
  /// call [TransparencyScaffold.geometryOf] in [State.didChangeDependencies], compare the
  ...
  /// [TransparencyScaffold] so that the client widget gets rebuilt whenever the [hasDrawer]
  /// value changes.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  /// See also:
  ///
  ///  * [TransparencyScaffold.of], which provides access to the [TransparencyScaffoldState] object as a
  ...
  static bool hasDrawer(BuildContext context, {bool registerForUpdates = true}) {
    if (registerForUpdates) {
      ...
      final TransparencyScaffoldState? scaffold = context.findAncestorStateOfType<TransparencyScaffoldState>();
      return scaffold?.hasDrawer ?? false;
    }
  }

  @override
  TransparencyScaffoldState createState() => TransparencyScaffoldState();
}

/// State for a [TransparencyScaffold].
///
/// Can display [BottomSheet]s. Retrieve a [TransparencyScaffoldState] from the current
/// [BuildContext] using [TransparencyScaffold.of].
class TransparencyScaffoldState extends State<TransparencyScaffold> with TickerProviderStateMixin, RestorationMixin {
  @override
  ...
  /// Whether this scaffold has a non-null [TransparencyScaffold.appBar].
  bool get hasAppBar => widget.appBar != null;

  /// Whether this scaffold has a non-null [TransparencyScaffold.drawer].
  bool get hasDrawer => widget.drawer != null;

  /// Whether this scaffold has a non-null [TransparencyScaffold.endDrawer].
  bool get hasEndDrawer => widget.endDrawer != null;

  /// Whether this scaffold has a non-null [TransparencyScaffold.floatingActionButton].
  bool get hasFloatingActionButton => widget.floatingActionButton != null;

  double? _appBarMaxHeight;

  /// The max height the [TransparencyScaffold.appBar] uses.
  ///
  ...
  /// Whether the [TransparencyScaffold.drawer] is opened.
  ///
  /// See also:
  ///
  ///  * [TransparencyScaffoldState.openDrawer], which opens the [TransparencyScaffold.drawer] of a
  ///    [TransparencyScaffold].
  bool get isDrawerOpen => _drawerOpened.value;

  /// Whether the [TransparencyScaffold.endDrawer] is opened.
  ///
  /// See also:
  ///
  ///  * [TransparencyScaffoldState.openEndDrawer], which opens the [TransparencyScaffold.endDrawer] of
  ///    a [TransparencyScaffold].
  ...
  ///
  /// If the scaffold has a non-null [TransparencyScaffold.drawer], this function will cause
  /// the drawer to begin its entrance animation.
  ///
  /// Normally this is not needed since the [TransparencyScaffold] automatically shows an
  /// appropriate [IconButton], and handles the edge-swipe gesture, to show the
  /// drawer.
  ///
  /// To close the drawer, use either [TransparencyScaffoldState.closeDrawer] or
  /// [Navigator.pop].
  ///
  /// See [TransparencyScaffold.of] for information about how to obtain the [TransparencyScaffoldState].
  void openDrawer() {
  ...
  ///
  /// If the scaffold has a non-null [TransparencyScaffold.endDrawer], this function will cause
  ...
  /// drawer.
  ///
  /// To close the drawer, use either [TransparencyScaffoldState.closeEndDrawer] or
  /// [Navigator.pop].
  ///
  /// See [TransparencyScaffold.of] for information about how to obtain the [TransparencyScaffoldState].
  void openEndDrawer() {
  ...
  /// Closes [TransparencyScaffold.drawer] if it is currently opened.
  ///
  /// See [TransparencyScaffold.of] for information about how to obtain the [TransparencyScaffoldState].
  void closeDrawer() {
  ...
  /// Closes [TransparencyScaffold.endDrawer] if it is currently opened.
  ///
  /// See [TransparencyScaffold.of] for information about how to obtain the [TransparencyScaffoldState].
  void closeEndDrawer() {
  ...
  /// Shows a Material Design bottom sheet in the nearest [TransparencyScaffold]. To show
  /// a persistent bottom sheet, use the [TransparencyScaffold.bottomSheet].
  ///
  ...
  /// [ModalRoute] and a back button is added to the app bar of the [TransparencyScaffold]
  ...
  /// [TransparencyScaffold.bottomSheet] constructor parameter.
  ///
  /// A persistent bottom sheet shows information that supplements the primary
  /// content of the app. A persistent bottom sheet remains visible even when
  /// the user interacts with other parts of the app.
  ...
  ///    sheet.
  ///  * [TransparencyScaffold.of], for information about how to obtain the [TransparencyScaffoldState].
  ...
  /// [TransparencyScaffold.floatingActionButton].
  ...
  /// Shows the [TransparencyScaffold.floatingActionButton].
  ...
  @protected
  @override
  void didUpdateWidget(TransparencyScaffold oldWidget) {
    super.didUpdateWidget(oldWidget);
...
class _DismissDrawerAction extends DismissAction {
  ...
  bool isEnabled(DismissIntent intent) {
    return TransparencyScaffold.of(context).isDrawerOpen || TransparencyScaffold.of(context).isEndDrawerOpen;
  }

  @override
  void invoke(DismissIntent intent) {
    TransparencyScaffold.of(context).closeDrawer();
    TransparencyScaffold.of(context).closeEndDrawer();
  }
}

/// An interface for controlling a feature of a [TransparencyScaffold].
///
/// Commonly obtained from [ScaffoldMessengerState.showSnackBar] or
/// [TransparencyScaffoldState.showBottomSheet].
...
class _StandardBottomSheetState extends State<_StandardBottomSheet> {
  ...
  bool extentChanged(DraggableScrollableNotification notification) {
    ...
    final TransparencyScaffoldState scaffold = TransparencyScaffold.of(context);
...
///
/// This is the type of objects returned by [TransparencyScaffoldState.showBottomSheet].
///
...
/// [TransparencyScaffold.bottomSheet].
