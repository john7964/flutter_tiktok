...
class FlutterAliListPlayer extends FlutterAliplayer {
  ...
  Future<void> removeSource(String uid) async {
    return FlutterAliPlayerFactory.methodChannel.invokeMethod(
      "removeSource",
      wrapWithPlayerId(arg: uid),
    );
  }

  /// 获取当前播放资源的uid
  Future<dynamic> getCurrentUid() {
    return FlutterAliPlayerFactory.methodChannel.invokeMethod("getCurrentUid", wrapWithPlayerId());
  }

  /// 清除播放列表
  Future<void> clear() async {
    return FlutterAliPlayerFactory.methodChannel.invokeMethod("clear", wrapWithPlayerId());
  }
  ...
  Future<void> setDefinition(String definition) {
    return FlutterAliPlayerFactory.methodChannel.invokeMethod(
      "setDefinition",
      wrapWithPlayerId(arg: definition),
    );
  }

  /// 当前位置移动到下一个进行准备播放
  /// 没有入参是url播放方式；有入参是sts播放方式，需要更新sts信息
  Future<void> moveToNext({String? accId, String? accKey, String? token, String? region}) async {
  ...
  /// 当前位置移动到上一个进行准备播放
  /// 没有入参是url播放方式；有入参是sts播放方式，需要更新sts信息
  Future<void> moveToPre({String? accId, String? accKey, String? token, String? region}) async {
    Map<String, dynamic> info = {
      'accId': accId,
      'accKey': accKey,
      'token': token,
