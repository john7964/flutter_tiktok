...
class NestedScrollViewOuter extends OriginalNestedScrollView {
  ...
  static OriginalOverlapAbsorberHandle sliverOverlapAbsorberHandleFor(BuildContext context) {
    final _OriginalInheritedNestedScrollView? target =
        context.dependOnInheritedWidgetOfExactType<_OriginalInheritedNestedScrollView>();
    assert(
      target != null,
  ...
  List<Widget> _buildSlivers(
    ...
    assert(headerSlivers.isNotEmpty, 'headerSliverBuilder must return at least one sliver.');
    final lastSliver = headerSlivers.removeLast();
    return <Widget>[
      ...headerSlivers,
      // ignore: deprecated_member_use_from_same_package
      OverlapAbsorberPlus(overscrollBehavior: OverscrollBehavior.outer, sliver: lastSliver),
      SliverFillRemaining(
...
class NestedScrollViewPlusOuterState extends NestedScrollViewStatePlus {
  ...
  Widget build(BuildContext context) {
    const defaultPhysics = BouncingScrollPhysics(parent: AlwaysScrollableScrollPhysics());
...
class _NestedScrollControllerOuter extends _OriginalNestedScrollController {
  ...
  Iterable<_OriginalNestedScrollPosition> get _debugNestedPositions {
    return Iterable.castFrom<ScrollPosition, _OriginalNestedScrollPosition>(positions);
  }

  Iterable<_OriginalNestedScrollPosition> get _releaseNestedPositions sync* {
    yield* Iterable.castFrom<ScrollPosition, _OriginalNestedScrollPosition>(positions);
  }
}
...
class _NestedScrollCoordinatorOuter extends _OriginalNestedScrollCoordinator {
  ...
  double unnestOffset(double value, _OriginalNestedScrollPosition source) {
    if (source.debugLabel == 'outer') {
      return value.clamp(-1 * double.infinity, _outerPosition!.maxScrollExtent);
    }
    // outer is scrolling
    if (_outerPosition!.maxScrollExtent - _outerPosition!.pixels > precisionErrorTolerance) {
      // coordinator offset = outer.value
      return _outerPosition!.pixels.clamp(-1 * double.infinity, _outerPosition!.maxScrollExtent);
    }
    // inner is scrolling
...
class _NestedOuterBallisticScrollActivityOuter extends _NestedBallisticScrollActivityOuter {
...
class _NestedInnerBallisticScrollActivityOuter extends _NestedBallisticScrollActivityOuter {
...
class SliverOverlapAbsorberOuter extends OriginalSliverOverlapAbsorber {
  const SliverOverlapAbsorberOuter({super.key, required super.handle, super.sliver});

  @override
  OriginalRenderSliverOverlapAbsorber createRenderObject(BuildContext context) {
    return RenderSliverOverlapAbsorberOuter(handle: handle);
  }
}
...
class RenderSliverOverlapAbsorberOuter extends OriginalRenderSliverOverlapAbsorber {
  RenderSliverOverlapAbsorberOuter({required super.handle, super.sliver});
  ...
  void performLayout() {
    ...
    final scrollExtend = maxExtent - minExtent;

    final layoutExtent = childLayoutGeometry.layoutExtent;
    geometry = childLayoutGeometry.copyWith(scrollExtent: scrollExtend, layoutExtent: layoutExtent);
    handle._setExtents(0, 0);
  }
}
...
//   const SliverOverlapInjectorOuter({super.key, required super.handle, super.sliver});
//
//   @override
//   OriginalRenderSliverOverlapInjector createRenderObject(BuildContext context) {
//     return RenderSliverOverlapInjectorOuter(handle: handle);
//   }
// }
//
// @Deprecated('No longer needed as of v2.0. Remove the Injector widget from atop your scroll views.')
// class RenderSliverOverlapInjectorOuter extends OriginalRenderSliverOverlapInjector {
//   RenderSliverOverlapInjectorOuter({required super.handle});
// }
